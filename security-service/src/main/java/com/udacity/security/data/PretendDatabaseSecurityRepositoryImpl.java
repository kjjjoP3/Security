...
/**
 * Fake repository implementation for demo purposes. Stores state information in
 * local memory and writes it to user preferences between app loads. This
 * implementation is intentionally a little hard to use in unit tests, so watch
 * out!
 */
public class PretendDatabaseSecurityRepositoryImpl implements SecurityRepository {
	...
	private static final Gson gson = new GsonBuilder().registerTypeAdapter(Sensor.class, new SensorInstanceCreator())
			.create(); // used to serialize objects into JSON
	...
	public PretendDatabaseSecurityRepositoryImpl() {
		...
		// we've serialized our sensor objects for storage, which should be a good
		// warning sign that
